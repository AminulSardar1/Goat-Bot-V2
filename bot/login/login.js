...
async function startBot(loginWithEmail) {
        ...
        (async function loginBot(appState) {
                ...
                try {
                        // Use the new aminul-new-fca login with account.txt
                        const api = await loginWithAccountTxt(dirAccount, global.GoatBot.config.optionsFca);
                        
                        // Automatic cookie refresh setup (optional)
                                ...
                                if (facebookAccount.email && facebookAccount.password) {
                                        ...
                                        setTimeout(async function refreshCookie() {
                                                try {
                                                        ...
                                                        // Use aminul login for refresh too
                                                        await loginWithAccountTxt(dirAccount, global.GoatBot.config.optionsFca);
                        ...
                        // Handle login errors with aminul-new-fca
                        log.err("LOGIN FACEBOOK", getText('login', 'loginError'), error.message);
                        global.statusAccountBot = 'can\'t login';
                        
                        // If email/password available, try alternative login
                        if (facebookAccount.email && facebookAccount.password) {
                                log.info("LOGIN FACEBOOK", "Attempting alternative login method...");
...
// Use the complete aminul-new-fca implementation
const { startBotWithAminul } = require('./aminulCompleteLogin.js');
